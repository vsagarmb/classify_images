...
def calculates_results_stats(results_dic):
    ...
    for _, value in results_dic.items():
        # Number of Dog Images - B
        if(value[3] == 1):
            n_dogs_img += 1
        # Number of Not Dog Images - D
        else:
            n_notdogs_img += 1
        
        # Number of Correct Dog matches - A
        if((value[3] == 1) and (value[4] == 1)):
            n_correct_dogs += 1

        # Number of Correct Non-Dog matches - C
        if((value[3] == 0) and (value[4] == 0)):
            n_correct_notdogs += 1

        # Number of label matches - Y
        if(value[2] == 1):
            n_match += 1

        # Number of Correct Breed matches - E
